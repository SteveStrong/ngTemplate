...
export class foShape2D extends foGlyph2D {
  ...
  public updateGlue(pnt?: any) {
    const point = pnt ? pnt : this.getLocation();
  ...
  public establishGlue(
    sourceName: string,
    target: foShape2D,
    targetName?: string
  ) {
    const glue = this.getGlue(
      `${this.myGuid}:${sourceName}->${target.myGuid}:${targetName}`
    );
    glue.glueTo(sourceName, target, targetName);
    return glue;
  }

  public glueConnectionPoints(
    target: foShape2D,
    sourceHandle?: string,
    targetHandle?: string
  ) {
    const glue = this.establishGlue(
      sourceHandle ? sourceHandle : shape2DNames.center,
      target,
      targetHandle ? targetHandle : shape2DNames.center
    );
    return glue;
  }
  ...
  protected generateConnectionPoints(
    spec: Array<any>,
    proxy?: Array<any>
  ): foCollection<foConnectionPoint2D> {
    let i = 0;
    if (!this._connectionPoints) {
      this._connectionPoints = new foCollection<foConnectionPoint2D>();
      spec.forEach(item => {
        const type = item.myType
          ? item.myType
          : RuntimeType.define(foConnectionPoint2D);
    ...
    const spec = [
      {
        x: w / 2,
        y: 0,
        myName: shape2DNames.top,
        myType: RuntimeType.define(foConnectionPoint2D)
      },
